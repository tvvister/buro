...
import driver_core
from driver_core.driver import DriverBase, DriverIml
...
class RestAPIServer:
    def __init__(self, driver):
        assert isinstance(driver, DriverBase), '<device> arg must be a type which is derived from DeviceBase'
        self._driver = driver
        self.app = web.Application()
        self.app.router.add_post('/start_channel/{channel:\d+}/{current:.*}/{voltage:.*}', self.start_channel)
    ...
    async def get_values(self, request):
        ...
        try:
            ...
            return web.json_response(values, dumps=partial(json.dumps, default=default_json))
        except Exception as e:
            resp = web.Response(text='{}'.format(e))
            resp.status = 400
            return resp
