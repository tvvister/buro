...
class DriverIml(DriverBase):
    def __init__(self, device):
        assert isinstance(
            device, DeviceBase), '<device> arg must be a type which is derived from DeviceBase'
        self._device = device
    ...
    async def start_channel(self, channel, current, voltage):
        assert isinstance(
            channel, int) and channel >= 1 and channel <= CHANNEL_COUNT, 'Channel must be a value from 1,2,3,4'
    ...
    async def get_values(self, channel=None):
        assert channel is None or (isinstance(channel, int) and channel >=
                                   1 and channel <= CHANNEL_COUNT), 'Channel must be a value from 1,2,3,4'
        results = {}
        for channel_num in (range(1, CHANNEL_COUNT + 1) if channel is None else [channel]):
            resp = await self._device.execute(':MEASure{}:CURRent?'.format(channel_num))
            _, current = resp
            time_v, voltage = await self._device.execute(':MEASure{}:VOLTage?'.format(channel_num))
            results[channel_num] = {'current': current,
                                    'time': time_v, 'voltage': voltage}
        return results
